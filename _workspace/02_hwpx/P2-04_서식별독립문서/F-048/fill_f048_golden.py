"""F-048 템플릿에 비식별 테스트값을 채워 골든 결과를 생성한다.
개인정보 응답란(이름/보호자 성명/전화번호)은 절대 채우지 않는다(토큰 자체가
없으므로 이 스크립트가 손댈 방법도 없음 — 원문 그대로 유지됨).
사용법: python fill_f048_golden.py <템플릿.hwpx> <골든결과.hwpx>
"""
import os
import shutil
import sys
...
def main():
    ...
    work_dir = os.path.join(os.path.dirname(out_path), "_build_tmp_golden_f048")
    ...
    shutil.rmtree(work_dir)
    print(f"PASS: F-048 골든결과 생성 완료 -> {out_path}")
