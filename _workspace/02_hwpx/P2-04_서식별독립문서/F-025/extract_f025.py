"""F-025(교복 A/S 계획서, [서식 9]) 원본 보존형 독립 HWPX 추출.

원본 sec 직계 자식 인덱스 623(F-025 본문 1개 문단, idx622 "[9-14]" 캡션
제외, idx624 "[9-15]"는 F-026/범위 제외 다음 서식 캡션)만 남기고 독립
문서로 분리한다. 1x1 CELL 표(A/S 계획 작성란)가 본문 전면을 차지하는
단일 구조 - 캐리어 병합 기법 적용(빈 2쪽 버그 예방).

허용 반영 범위: 학교명(C-01, "◯◯◯◯학교장", U+25EF, 말미 서명
"◯◯◯◯학교장 귀하" 1회)만 토큰화한다. 업체 상호는 원문에서도 빈칸.

사용법: python extract_f025.py <원본.hwpx> <출력_템플릿.hwpx>
"""
import copy
import os
import shutil
...
def main():
    ...
    work_dir = os.path.join(os.path.dirname(out_path), "_build_tmp_f025")
    ...
    if len(sec_children) < 625:
        raise AssertionError(f"예상보다 문단 수가 적음: {len(sec_children)}")

    caption_text = "".join(
        t.text or "" for t in sec_children[622].iter() if etree.QName(t.tag).localname == "t"
    )
    if "[ 9-14 ]" not in caption_text:
        raise AssertionError(f"idx622가 F-025 캡션이 아님: {caption_text!r}")
    next_caption_text = "".join(
        t.text or "" for t in sec_children[624].iter() if etree.QName(t.tag).localname == "t"
    )
    if "[ 9-15 ]" not in next_caption_text:
        raise AssertionError(f"idx624가 다음(F-026, 범위 제외) 캡션이 아님: {next_caption_text!r}")
    ...
    body = copy.deepcopy(sec_children[623])
    ...
    shutil.rmtree(work_dir)
    print(f"PASS: F-025 독립 템플릿 생성 완료 -> {out_path}")
