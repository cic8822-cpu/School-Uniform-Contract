"""F-023(납품 교복 제조 사양서, [서식 6]) 원본 보존형 독립 HWPX 추출.

원본 sec 직계 자식 인덱스 617(F-023 본문 1개 문단, idx616 "[9-11]" 캡션
제외, idx618 "[9-12]"는 다음 서식 F-024 캡션)만 남기고 독립 문서로
분리한다.
...
사용법: python extract_f023.py <원본.hwpx> <출력_템플릿.hwpx>
"""
import copy
import os
import shutil
...
def main():
    ...
    work_dir = os.path.join(os.path.dirname(out_path), "_build_tmp_f023")
    ...
    if "[ 9-11 ]" not in caption_text:
        raise AssertionError(f"idx616이 F-023 캡션이 아님: {caption_text!r}")
    next_caption_text = "".join(
        t.text or "" for t in sec_children[618].iter() if etree.QName(t.tag).localname == "t"
    )
    if "[ 9-12 ]" not in next_caption_text:
        raise AssertionError(f"idx618이 다음(F-024) 캡션이 아님: {next_caption_text!r}")
    ...
    body = copy.deepcopy(sec_children[617])
    ...
    shutil.rmtree(work_dir)
    print(f"PASS: F-023 독립 템플릿 생성 완료 -> {out_path}")
