"""F-024(교복 품목별 단가 비율표(업체 제출양식), [서식 7]) 원본 보존형
독립 HWPX 추출.

원본 sec 직계 자식 인덱스 619(F-024 본문 1개 문단, idx618 "[9-12]" 캡션
제외, idx620 "[9-13]"는 F-026/범위 제외 다음 서식 캡션)만 남기고 독립
문서로 분리한다.

주의: 이 서식은 F-019~023과 달리 학교명 placeholder가 U+25EF(◯)가
아니라 U+25CB(○, WHITE CIRCLE)를 사용한다 — 직접 코드포인트 확인으로
검증됨. 절대 혼동하지 않는다.

허용 반영 범위: 학교명(C-01, "○○○○학교장", U+25CB, 말미 서명 "○○○○학교장
귀하" 1회)만 토큰화한다. 학년도 placeholder는 없음(직접 확인). 업체 상호는
원문에서도 빈칸.

사용법: python extract_f024.py <원본.hwpx> <출력_템플릿.hwpx>
"""
import copy
import os
import shutil
...
def main():
    ...
    work_dir = os.path.join(os.path.dirname(out_path), "_build_tmp_f024")
    ...
    if len(sec_children) < 621:
        raise AssertionError(f"예상보다 문단 수가 적음: {len(sec_children)}")

    caption_text = "".join(
        t.text or "" for t in sec_children[618].iter() if etree.QName(t.tag).localname == "t"
    )
    if "[ 9-12 ]" not in caption_text:
        raise AssertionError(f"idx618이 F-024 캡션이 아님: {caption_text!r}")
    next_caption_text = "".join(
        t.text or "" for t in sec_children[620].iter() if etree.QName(t.tag).localname == "t"
    )
    if "[ 9-13 ]" not in next_caption_text:
        raise AssertionError(f"idx620이 다음(F-026, 범위 제외) 캡션이 아님: {next_caption_text!r}")

    body_pre = sec_children[619]
    body_text_check = "".join(
        t.text or "" for t in body_pre.iter() if etree.QName(t.tag).localname == "t"
    )
    if "○○○○학교" not in body_text_check:
        raise AssertionError(
            f"idx619 본문에 예상한 U+25CB('○○○○학교') 표기가 없음 - 원문자 종류 재확인 필요: {body_text_check!r}"
        )
    if "◯◯◯◯학교" in body_text_check:
        raise AssertionError("idx619 본문에 U+25EF('◯◯◯◯학교')가 섞여 있음 - 재확인 필요")
    ...
    body = copy.deepcopy(sec_children[619])
    ...
    shutil.rmtree(work_dir)
    print(f"PASS: F-024 독립 템플릿 생성 완료 -> {out_path}")
