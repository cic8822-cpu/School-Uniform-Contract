"""F-029 템플릿에 비식별 테스트값을 채워 골든 결과를 생성한다.
개인정보 응답란(업체명/사업자등록번호/대표자/동의여부)은 원문 그대로
빈칸 유지 — 토큰 자체가 없으므로 이 스크립트가 손댈 방법도 없음.
사용법: python fill_f029_golden.py <템플릿.hwpx> <골든결과.hwpx>
"""
import os
import shutil
import sys
...
def main():
    ...
    work_dir = os.path.join(os.path.dirname(out_path), "_build_tmp_golden_f029")
    ...
    shutil.rmtree(work_dir)
    print(f"PASS: F-029 골든결과 생성 완료 -> {out_path}")
