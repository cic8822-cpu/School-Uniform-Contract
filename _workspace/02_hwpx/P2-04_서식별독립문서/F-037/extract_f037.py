"""F-037(평가위원회 참석 등록부) 원본 보존형 독립 HWPX 추출.

원본 sec 직계 자식 인덱스 737~743(F-037 본문, idx736 "[13-1]" 캡션 제외)만
남기고 독립 문서로 분리한다. 다중 행/열 표(10x5)라 F-002/F-003 계열의 위험한
고정폭 단일 셀 표가 아님.

허용 반영 범위: 학년도(C-02)만 토큰화한다(이 서식엔 학교명 placeholder가
없음, 원본 대조로 확인). 회의 일시·장소·참석자 서명란은 이 회차 고유의
행사 데이터라 정적 HWPX 단계에서는 원문 유지.

사용법: python extract_f037.py <원본.hwpx> <출력_템플릿.hwpx>
"""
import copy
import os
import shutil
...
def main():
    ...
    if "13-1" not in caption_text:
        raise AssertionError(f"idx736이 F-037 캡션이 아님: {caption_text!r}")
    ...
    body_paragraphs = [copy.deepcopy(sec_children[i]) for i in range(737, 744)]
    ...
    def np(orig_idx):
        return new_children[1 + (orig_idx - 737)]

    replace_text_anywhere(np(738), "20○○학년도", "{{학년도}}")
    ...
    shutil.rmtree(work_dir)
    print(f"PASS: F-037 독립 템플릿 생성 완료 -> {out_path}")
