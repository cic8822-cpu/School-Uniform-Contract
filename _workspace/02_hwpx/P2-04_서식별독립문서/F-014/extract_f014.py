"""F-014(제안서 평가항목 및 배점기준) 원본 보존형 독립 HWPX 추출.

원본 sec 직계 자식 인덱스 580~585(F-014 본문, idx579 "[9-3]" 캡션 제외,
idx586 "[9-4]"는 다음 서식 F-015 캡션)만 남기고 독립 문서로 분리한다.
29x9 CELL 표(평가항목·배점기준)+1x1 CELL 표(상세 설명) 구조.

원본을 직접 확인한 결과 **이 서식은 학교명·학년도 placeholder가 전혀
없음**(순수 평가기준 예시 텍스트, 어느 학교에도 공통 적용되는 배점표라
학교 식별 정보 자체가 불필요함을 확인 — 추측 금지 원칙에 따라 전수
스캔으로 확인). 따라서 토큰화 없이 원문 그대로 독립 문서로 분리만 한다.

사용법: python extract_f014.py <원본.hwpx> <출력_템플릿.hwpx>
"""
import copy
import os
import shutil
...
def main():
    ...
    work_dir = os.path.join(os.path.dirname(out_path), "_build_tmp_f014")
    ...
    if "[ 9-3 ]" not in caption_text:
        raise AssertionError(f"idx579가 F-014 캡션이 아님: {caption_text!r}")
    next_caption_text = "".join(
        t.text or "" for t in sec_children[586].iter() if etree.QName(t.tag).localname == "t"
    )
    if "[ 9-4 ]" not in next_caption_text:
        raise AssertionError(f"idx586이 다음(F-015) 캡션이 아님: {next_caption_text!r}")
    ...
    body_paragraphs = [copy.deepcopy(sec_children[i]) for i in range(580, 586)]
    ...
    # 학교명/학년도 placeholder가 없으므로 토큰 치환 없음. 방어적 재확인만 수행.
    all_text = "".join(t.text or "" for t in sec_elem.iter() if etree.QName(t.tag).localname == "t")
    for stray_check in ["○ ○ 학 교", "○○학교", "◯◯◯◯학교"]:
        assert stray_check not in all_text, f"예상 못한 학교명 placeholder 발견(직접 재확인 필요): {stray_check!r}"
    ...
    shutil.rmtree(work_dir)
    print(f"PASS: F-014 독립 템플릿 생성 완료 -> {out_path}")
