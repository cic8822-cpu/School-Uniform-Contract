"""F-050 템플릿에 비식별 테스트값을 채워 골든 결과를 생성한다.
설문 응답 칸(합계/평균 점수)·기타 의견 서술란은 원문에서도 이미 빈칸이며
절대 채우지 않는다(토큰 자체가 없으므로 이 스크립트가 손댈 방법도 없음).
사용법: python fill_f050_golden.py <템플릿.hwpx> <골든결과.hwpx>
"""
import os
import shutil
import sys
...
def main():
    ...
    work_dir = os.path.join(os.path.dirname(out_path), "_build_tmp_golden_f050")
    ...
    for blank in ["합계 (     )점", "평균 (     )점"]:
        assert blank in remaining, f"설문 응답 칸이 예상과 달라짐: {blank!r}"
    ...
    shutil.rmtree(work_dir)
    print(f"PASS: F-050 골든결과 생성 완료 -> {out_path}")
