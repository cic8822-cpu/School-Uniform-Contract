"""F-003 템플릿(토큰 삽입 완료)에 비식별 테스트값을 채워 골든 결과를 생성한다.
F-001/F-002와 동일 원칙: kordoc fill의 라벨/값 추론에 의존하지 않고, 템플릿에
이미 삽입된 {{토큰}} 문자열을 문서 전체에서 직접 치환한다.

사용법: python fill_f003_golden.py <템플릿.hwpx> <골든결과.hwpx>
"""
import os
import shutil
import sys
...
def main():
    ...
    shutil.rmtree(work_dir)
    print(f"PASS: F-003 골든 결과 생성 완료 -> {out_path} (치환 횟수: {replaced_total})")
