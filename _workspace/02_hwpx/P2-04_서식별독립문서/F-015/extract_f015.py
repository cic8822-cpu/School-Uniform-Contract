"""F-015(정량적 평가, [1단계]) 원본 보존형 독립 HWPX 추출.

원본 sec 직계 자식 인덱스 587~590(F-015 본문, idx586 "[9-4]" 캡션 제외,
idx591 "[9-5]"는 다음 서식 F-016 캡션)만 남기고 독립 문서로 분리한다.
16x5 CELL 표(평가항목+심사자 서약 문구 포함) 단일 구조.

원본에서 직접 확인한 결과, 표 안의 서약 문구에 "본인은 2026학년도 ○○학교
교복 학교주관 교복 구매업체 선정을 위한..."이 있는데, **"2026"이 20○○
같은 명시적 placeholder 문자 없이 이미 확정 숫자로 고정돼 있음**을 확인함
(원본 매뉴얼 표지 자체가 "2026. 9." 작성본이라 예시 연도가 그대로 남은
것으로 추정 — 8.2절 공통 교훈 6번: 원문에 없는 치환을 임의로 만들지
않는다는 원칙에 따라 "2026"은 토큰화하지 않고 원문 그대로 유지함).
학교명("○○학교", 2회)만 토큰화한다.

사용법: python extract_f015.py <원본.hwpx> <출력_템플릿.hwpx>
"""
import copy
import os
import shutil
...
def main():
    ...
    work_dir = os.path.join(os.path.dirname(out_path), "_build_tmp_f015")
    ...
    if len(sec_children) < 592:
        raise AssertionError(f"예상보다 문단 수가 적음: {len(sec_children)}")

    caption_text = "".join(
        t.text or "" for t in sec_children[586].iter() if etree.QName(t.tag).localname == "t"
    )
    if "[ 9-4 ]" not in caption_text:
        raise AssertionError(f"idx586이 F-015 캡션이 아님: {caption_text!r}")
    next_caption_text = "".join(
        t.text or "" for t in sec_children[591].iter() if etree.QName(t.tag).localname == "t"
    )
    if "[ 9-5 ]" not in next_caption_text:
        raise AssertionError(f"idx591이 다음(F-016) 캡션이 아님: {next_caption_text!r}")
    ...
    body_paragraphs = [copy.deepcopy(sec_children[i]) for i in range(587, 591)]
    ...
    def np(orig_idx):
        return new_children[1 + (orig_idx - 587)]

    replace_text_anywhere(np(590), "○○학교", "{{학교명}}", expected_count=2)
    ...
    shutil.rmtree(work_dir)
    print(f"PASS: F-015 독립 템플릿 생성 완료 -> {out_path}")
